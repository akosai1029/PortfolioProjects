...
SELECT
	continent,
	SUM(CAST(total_cases as bigint)) as total_cases,				-- use SUM function to add up the data and be able to use GROUP BY function
	SUM(CAST(total_deaths as bigint)) as total_deaths				-- change data type NVARCHAR to BIGINT to use the SUM aggregate function
FROM PortfolioProjectSQL001..CovidDeaths
...
FROM PortfolioProjectSQL001..CovidDeaths
WHERE location NOT IN (																-- SELECT statement can also be used in WHERE clause as subquery
